package com.cardinalhealth.kata.functions;

import com.cardinalhealth.kata.domain.Card;
import com.cardinalhealth.kata.domain.Ranks;
import com.cardinalhealth.kata.domain.Suit;
import org.junit.jupiter.api.Assertions;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;

import java.util.ArrayList;
import java.util.List;
import java.util.Map;

public class ThreeOfAKindFunctionTest {

    @Test
    @DisplayName("Poker hand with  3-of-a-kind should return THREE_OF_A_KIND")
    public void testApply(){
        final Card c1 = new Card(Suit.CLUB, "2");
        final Card c2 = new Card(Suit.CLUB, "2");
        final Card c3 = new Card(Suit.CLUB, "2");
        final Card c4 = new Card(Suit.CLUB, "3");
        final Card c5 = new Card(Suit.HEARTS, "4");

        final List<Card> cards = new ArrayList<>();
        cards.add(c1);
        cards.add(c2);
        cards.add(c3);
        cards.add(c4);
        cards.add(c5);
        final Map.Entry<Ranks, Long> actual = new ThreeOfAKindFunction().apply(cards);

        Assertions.assertEquals(3, actual.getValue());
        Assertions.assertEquals(Ranks.THREE_OF_A_KIND, actual.getKey());
    }

    @Test
    @DisplayName("Poker hand with no 3-of-a-kind should return UNKNOWN")
    public void testDoesNotApply(){
        final Card c1 = new Card(Suit.CLUB, "3");
        final Card c2 = new Card(Suit.CLUB, "4");
        final Card c3 = new Card(Suit.CLUB, "5");
        final Card c4 = new Card(Suit.CLUB, "J");
        final Card c5 = new Card(Suit.HEARTS, "Q");

        final List<Card> cards = new ArrayList<>();
        cards.add(c1);
        cards.add(c2);
        cards.add(c3);
        cards.add(c4);
        cards.add(c5);
        final Map.Entry<Ranks, Long> actual = new ThreeOfAKindFunction().apply(cards);

        Assertions.assertEquals(0, actual.getValue());
        Assertions.assertEquals(Ranks.UNKNOWN, actual.getKey());
    }
}
